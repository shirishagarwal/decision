                ...
                <main className="max-w-7xl mx-auto py-12 px-6">
                    ...
                    <div className="grid lg:grid-cols-3 gap-12">
                        <div className="lg:col-span-2 space-y-8">
                            ...
                            {step === 1 && (
                                <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-200/50 animate-in">
                                    ...
                                    <div className="space-y-8">
                                        ...
                                        <button onClick={analyzeContext} disabled={!title || isAnalyzing} className="w-full bg-slate-900 text-white py-6 rounded-2xl font-black text-lg shadow-2xl hover:bg-indigo-600 transition-all flex items-center justify-center gap-3">
                                            {isAnalyzing && <Icon name="loader-2" className="animate-spin" />}
                                            {isAnalyzing ? 'Analyzing Failures...' : 'Analyze Strategic Gaps'}
                                        </button>
                                    </div>
                                </div>
                            )}
                            ...
                            {step === 2 && (
                                <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-200/50 animate-in">
                                    <h2 className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-10">02 • Intelligence Gaps</h2>
                                    {gaps.length > 0 ? (
                                        <div className="space-y-8">
                                            ...
                                            <div className="flex gap-4">
                                                ...
                                                <button onClick={analyzeContext} className="flex-1 bg-indigo-600 text-white py-5 rounded-2xl font-black">Refresh Recommendations</button>
                                            </div>
                                            <div className="text-center">
                                                <button onClick={() => setStep(3)} className="text-xs font-black text-slate-300 uppercase tracking-widest hover:text-indigo-600 transition">Skip & View Speculative Paths</button>
                                            </div>
                                        </div>
                                    ) : (
                                        ...
                                        {options.map((opt) => (
                                            <div key={opt.id} className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm relative group">
                                                ...
                                                <div className="flex justify-between items-start mb-6 mr-10">
                                                    ...
                                                    {opt.isAiGenerated && <span className="bg-emerald-50 text-emerald-600 text-[8px] px-2 py-1 rounded-full font-black uppercase tracking-widest">AI SUGGESTED</span>}
                                                </div>
